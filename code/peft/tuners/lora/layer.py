        ...
        if self.disable_adapters:
            ...
            if self.other_adapter is not None:
                for i,tmp_name in enumerate(self.other_adapter):
                    ...
                    tmp_scaling = self.scaling[tmp_name]
                    tmp_result = tmp_lora_B(tmp_lora_A(tmp_dropout(x))) * tmp_scaling
                    tmp_result = tmp_result.detach()
                    result += tmp_result * self.adapter_weight[i+1]
            tmp_weight = 1 if self.other_adapter is None else self.adapter_weight[0]
            result += lora_B(lora_A(dropout(x))) * scaling * tmp_weight
...
class Embedding(nn.Embedding, LoraLayer):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            # if self.other_adapter is not None:
            #     #print("***************************other lora**************************")
            #     embedding_A = embedding_A * self.adapter_weight[0]
            #     for i,tmp_name in enumerate(self.other_adapter):
            #         tmp_embedding_A = self.lora_embedding_A[tmp_name].T
            #         tmp_embedding_B = self.lora_embedding_B[tmp_name].T
            #         embedding_A += tmp_embedding_A.detach() * self.adapter_weight[i+1]
            #         embedding_B += tmp_embedding_B.detach()
            if self.other_adapter is not None:
                for i,tmp_name in enumerate(self.other_adapter):
                    tmp_embedding_A = self.lora_embedding_A[tmp_name].T
                    tmp_embedding_B = self.lora_embedding_B[tmp_name].T
                    tmp_scaling = self.scaling[tmp_name]
                    tmp_after_A = self._embed(x, tmp_embedding_A)
                    tmp_result = (tmp_after_A @ tmp_embedding_B) * tmp_scaling
                    tmp_result = tmp_result.detach()
                    result += tmp_result * self.adapter_weight[i+1]
            tmp_weight = 1 if self.other_adapter is None else self.adapter_weight[0]
        
            after_A = self._embed(x, embedding_A)
            result += (after_A @ embedding_B) * scaling * tmp_weight
